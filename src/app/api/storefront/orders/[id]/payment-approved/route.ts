...
import { markStorefrontOrderPaymentApproved } from "@/lib/storefront/service";
...
export async function POST(
  ...
  const rateLimit = checkRateLimit(request, {
    key: "storefront:payment-approved",
    limit: 120,
    windowMs: 60 * 1000,
  });
  ...
  try {
    ...
    const response = await markStorefrontOrderPaymentApproved(
      access.channelId,
      id,
      body,
    );
