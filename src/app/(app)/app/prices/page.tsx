...
import { PRICE_PAGE_ENABLED, STOCK_ACCOUNTING_ENABLED } from "@/lib/features";
...
const resolveDefaultPriceList = (priceLists: PriceListOption[]) =>
  priceLists.find((priceList) => priceList.isDefault) ?? priceLists[0] ?? null;
  ...
  const defaultPrice =
    defaultPriceList === null
      ...
      : calculatePriceFromPercentage(
          cost,
          parseNumber(percentages[defaultPriceList.id]),
        );

  for (const priceList of priceLists) {
    const percentage = parseNumber(percentages[priceList.id]);
    const basePrice =
      priceList.id === defaultPriceList?.id ? cost : defaultPrice;
    computedPrices[priceList.id] = calculatePriceFromPercentage(
      basePrice,
      percentage,
    );
  }
...
const derivePercentagesFromPrices = (
  ...
  const defaultStoredPercentage =
    defaultPriceList === null
      ? null
      : parseNumber(getProductPercentageForList(product, defaultPriceList));
  const defaultListPrice =
    defaultPriceList === null
      ? null
      : parseNumber(getProductPriceForList(product, defaultPriceList));
  const defaultBasePrice =
    defaultListPrice ??
    calculatePriceFromPercentage(effectiveCost, defaultStoredPercentage);
  ...
  for (const priceList of priceLists) {
    ...
    const basePrice =
      priceList.id === defaultPriceList?.id ? effectiveCost : defaultBasePrice;
...
const formatSaveQueueMessage = (summary: StockSaveQueueSummary) => {
  ...
  return `Guardando precios: ${parts.join(", ")}`;
};
...
export default function PricesPage() {
      ...
      try {
        ...
        if (!res.ok) {
          if (requestId === stockRequestIdRef.current) {
            setStatus("No se pudieron cargar precios");
          }
          return;
        }
  ...
  useEffect(() => {
    if (!PRICE_PAGE_ENABLED || !isSortOrderReady) return;
    loadStock({
      offset: 0,
      append: false,
  ...
  const publishSaveQueueSummary = () => {
    ...
    const message =
      summary.failed > 0
        ? `Precios con errores: ${summary.failed} no se pudieron guardar`
        : `Precios guardados: ${summary.saved} productos actualizados`;
    const toastOptions = {
      render: message,
  ...
  const finishSaveQueue = () => {
    ...
    setStatus(
      summary.failed > 0
        ...
        : `Precios guardados (${summary.saved} productos)`,
    );
  ...
  const executeStockSaveJob = async (job: StockSaveJob) => {
    if (job.costChanged || job.costUsdChanged || job.priceUpdates.length > 0) {
      const patchRes = await fetch("/api/prices", {
        method: "PATCH",
  ...
  const stockExitGuardDescription = hasSaveQueueActivity
    ? "Todavia hay actualizaciones en cola o enviandose. Espera a que terminen para evitar que los precios queden a mitad de camino."
